...
func (r *LocationRepository) GetAll(ctx context.Context, groupID uuid.UUID, filter LocationQuery) ([]LocationOutCount, error) {
	where := bqb.New("WHERE locations.group_locations = ?", groupID)
	sqlq := bqb.New(`
		SELECT
			id,
			name,
...
func (r *LocationRepository) GetOneByGroup(ctx context.Context, groupID, id uuid.UUID) (LocationOut, error) {
	return r.getOne(ctx, location.ID(id), location.HasGroupWith(group.ID(groupID)))
}
...
func (r *LocationRepository) UpdateByGroup(ctx context.Context, groupID, id uuid.UUID, data LocationUpdate) (LocationOut, error) {
	v, err := r.update(ctx, data, location.ID(id), location.HasGroupWith(group.ID(groupID)))
	if err != nil {
		return LocationOut{}, err
	}
...
func (r *LocationRepository) DeleteByGroup(ctx context.Context, groupID, id uuid.UUID) error {
	_, err := r.db.Location.Delete().Where(location.ID(id), location.HasGroupWith(group.ID(groupID))).Exec(ctx)
	if err != nil {
		return err
	}
	r.publishMutationEvent(groupID)
